...
!-------------------------------------------------------------------------------
  ...
  type physics_state
     ...
     real(r8), dimension(:,:),allocatable        :: &
          ...
          u_ref,       &! zonal wind (m/s)
          v_ref,       &! meridional wind (m/s)
          s,       &! dry static energy
...
! allocate the individual state components
  ...
  allocate(state%u_ref(psetcols,pver), stat=ierr)
  if ( ierr /= 0 ) call endrun('physics_state_alloc error: allocation error for state%u_ref')

  allocate(state%v_ref(psetcols,pver), stat=ierr)
  if ( ierr /= 0 ) call endrun('physics_state_alloc error: allocation error for state%v_ref')
...
! deallocate the individual state components
  ...
  deallocate(state%u_ref, stat=ierr)
  if ( ierr /= 0 ) call endrun('physics_state_dealloc error: deallocation error for state%u_ref')

  deallocate(state%v_ref, stat=ierr)
  if ( ierr /= 0 ) call endrun('physics_state_dealloc error: deallocation error for state%v_ref')
