...
contains
  ...
  subroutine model_init2(elem,hybrid,deriv,hvcoord,tl,nets,nete )
    ...
    do k=1,nlev
       ...
       !active for p<10*ptop
       !if (nu_scale_top(k)<0.15d0) nu_scale_top(k)=0
       
       !active for p < 5/3*ptop
       !if (nu_scale_top(k)<4.235d0) nu_scale_top(k)=0
       
       !active for p < 10/3*ptop
       if (nu_scale_top(k)<1.3211d0) nu_scale_top(k)=0

       !limit scale to factor of 8
